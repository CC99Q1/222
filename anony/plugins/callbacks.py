...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        return await anon.play_media(chat_id, msg, media, query.lang)
...
@app.on_message(
    filters.regex(r"^(وقف|توقف)(@\w+)?$") 
    & filters.group 
    & ~app.bl_users
)
@lang.language()
@admin_check
async def pause_command(_, m: types.Message):
    chat_id = m.chat.id
    ...
    await m.reply_text(m.lang["play_paused"].format(user))
